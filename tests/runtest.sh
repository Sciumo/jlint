...
do
	...
	# sort -d so that jlint will analyze the class files always in the same order. 
	# output of jlint will be written in $I.log
	# errors occuring during runtime are written into $I.err
	# names of the class files which produced an error in jlint are written into $I.errfiles
    fi
    I=`expr $I + 1`
done
